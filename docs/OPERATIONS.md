...
| `make sync-prospects-api` | Replaces the prospect board with a live pull from `/draft/prospects/{year}` — same ranked board as the CSV/no-R fallback, but live and with real `blurb`/scouting-video-link text (populated by MLB as of 2026-07-08; empty before then). `pre_draft_sync` tries this first now, falling back to baseballr → CSV → no-R unchanged if it fails. Sends a Telegram alert if the ranked board changed since the last sync (new entrants, dropped players, rank changes) — pass `--no-notify` to suppress. Every prospect-sync path (this one, `sync-prospects`, `seed-no-r-prospects`, `seed-prospects-csv`) fully clears the year's board first (`db.clear_prospect_board`), so switching which source succeeds between runs can never leave duplicate/mixed-source rows for the same player. The dashboard shows which source is currently active as a small badge under "Prospects Loaded" (also queryable directly: `SELECT DISTINCT source FROM prospects WHERE draft_year=?`). |
