#!/usr/bin/env bash
# Draft-day live polling loop: repeatedly runs `main.py live-monitor` on an
# interval and logs each run. Refuses to start a second overlapping poller
# against the same draft year, and stops itself after repeated failures
# instead of spamming retries forever.
#
# Env overrides:
...
log "starting live-monitor loop for $YEAR (interval ${INTERVAL}s, pidfile $PIDFILE)"

failures=0
while true; do
  if python3 main.py "${DB_ARGS[@]}" live-monitor --year "$YEAR" >>"$LOGFILE" 2>&1; then
    failures=0
  else
    failures=$((failures + 1))
