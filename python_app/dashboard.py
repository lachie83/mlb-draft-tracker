...
def fetch_dashboard_data(conn: sqlite3.Connection, year: int):
    top_250 = [
        ...
        for r in q(
            conn,
            """
            SELECT rank, full_name, position_name, school_name, draft_team_name, pick_number
            FROM prospects
    ...
    best_available = [
        ...
        for r in q(
            conn,
            """
            SELECT rank, full_name, position_name, school_name
            FROM prospects
    ...
    predictions = [
        ...
        for r in q(
            conn,
            """
            SELECT pick_number, team_name, player_name,
                   ROUND(predicted_probability, 4) AS predicted_probability,
                   model_version
            FROM predictions
            WHERE draft_year = ?
            ORDER BY pick_number, predicted_probability DESC
            LIMIT 100
            """,
            (year,),
        )
...
def app_factory(db_path: str):
    def app(environ, start_response):
        ...
        summary, top_250, best_available, picks, predictions = fetch_dashboard_data(conn, year)
        conn.close()
        ...
        <body>
          <h1>MLB Draft Tracker Dashboard</h1>
          <div class="year-nav">
            <a href="/?year=2025">View 2025</a>
            <a href="/?year=2026">View 2026</a>
          </div>
          <p class="muted">Showing draft year: {summary['year']}</p>
